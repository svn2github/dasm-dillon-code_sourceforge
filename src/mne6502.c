...
/*
    Illegal 6502 opcodes. These used to be mixed in with the
    official opcodes above, but they should really live in
    their own seperate box. Currently they are still added
    by default when "processor 6502" is selected, but that's
    likely to change. Sadly there's no "suite6502illegal.asm"
    yet, but there probably should be. Also I think the list
    here is incomplete? There's no "kil" for example. [phf]
*/
MNEMONIC Mne6502illegal[] = {
